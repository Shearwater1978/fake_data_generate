...
def curr_time():
    parent_logger.debug("Called function {message}".format(message=sys._getframe(0).f_code.co_name))
...
def generate_bulk(count, locale, json_file):
    ...
    """
    parent_logger.info("Called function {message}".format(message=sys._getframe(0).f_code.co_name))
    person = {}
    fake = Faker(locale)
    ...
    try:
        ...
        parent_logger.error("Something wrong in function {message}. Error message {error}".format(message=sys._getframe(0).f_code.co_name, error=e))
    return person
        ...
        if mode == 'headers':
            for count, item in enumerate(data['headers'], start=1):
                ...
                try:
                    ...
                    parent_logger.error("Json file is malformed. Error is: {message}".format(message=e))
        else:
            for count, item in enumerate(data['fields'], start=1):
                keyIdx = f'key{count}'.format(count)
                dict_key = data['fields'][keyIdx]['name']
...
def get_header_fields_name(mode, json_file):
    ...
    if mode == 'headers':
        try:
            ...
            parent_logger.error("Unable to execute get_header_fields_name. Error is: {message}".format(message=e))
...
def read_env():
    ...
    if os.getenv('LOCALE') in supportedLocale:
        ...
        LOCALE = 'ru_RU'
    parent_logger.debug("Env variable LOCALE set to: {message}".format(message=LOCALE))
    if os.getenv('PERSON_COUNT'):
        ...
        PERSON_COUNT = 10
    parent_logger.debug("Env variable PERSON_COUNT set to: {message}".format(message=PERSON_COUNT))
    if os.getenv('OUTPUT_FILE_NAME'):
        OUTPUT_FILE_NAME = os.getenv('OUTPUT_FILE_NAME')
    else:
        OUTPUT_FILE_NAME = 'default.csv'
    parent_logger.debug("Env variable OUTPUT_FILE_NAME set to: {message}".format(message=OUTPUT_FILE_NAME))
    if os.getenv('JSON_TEMPLATE_FILE'):
        JSON_TEMPLATE_FILE = os.getenv('JSON_TEMPLATE_FILE')
    else:
        JSON_TEMPLATE_FILE = 'headers.json'
    parent_logger.debug("Env variable JSON_TEMPLATE_FILE set to: {message}".format(message=JSON_TEMPLATE_FILE))
...
def main():
    if os.getenv('LOGGER_LEVEL_SEVERITY'):
        logLevel = os.getenv('LOGGER_LEVEL_SEVERITY')
    else:
        logLevel = 'INFO'
    ...
    try:
        ...
        parent_logger.error("Unable to execute Actions. Error: {message}".format(message=e))
